...
for STRAND in \$STRANDS; do
    for (( i = 0; i < \${#FASTAS[@]}; i++ )); do
        ...
        if [ \"\$STRAND\" == \"pos\" ]; then
            ...
            bioawk -v len=\"\$SEED_LENGTH\" -c header '{split(\$1,seed,\"|\"); print seed[1]\"\t\"len\"\t\"\$5}' \$POSNOTSIF > $SIF
        else
            python $SRC/aggregate_peaks.py $NEGPEAK $ANNOTATION \$SEED_FASTA $GDARCHIVE | gzip -c > \$NEGNOTSIF
            bioawk -v len=\"\$SEED_LENGTH\" -c header '{split(\$1,seed,\"|\"); print seed[1]\"\t\"len\"\t\"\$5}' \$NEGNOTSIF >> $SIF
        fi
