...
def get_count_files(bed, bedgraphs):
    ...
    for bg in bedgraphs:
        ...
        cmd = "|bedtools map -c 4 -o sum -null 0 -a %s -b %s" % (bed, bg)
        result_header = "chrom start stop name score strand count".split()
        for r in ts.reader(cmd, header=result_header):
            polya, gene = r['name'].split("|")
            tf.write("%s\t%s\t%s\n" % (gene, polya, r['count']))
        tf.close()
...
if __name__ == '__main__':
    ...
    p.add_argument("bed", help="reference bed of polya sites. the name field is polya|gene.")
